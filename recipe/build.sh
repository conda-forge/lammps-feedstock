...
if [ "$PYTHON_IMPL" != "PyPy" ]; then
  args=$args" -D MLIAP_ENABLE_PYTHON=ON -D PKG_PYTHON=ON -D Python_EXECUTABLE=${PYTHON} -D Python_COMPILER=${PYTHON} -D Python_INCLUDE_DIR=${PREFIX}/include/python${PY_VER}m -D Python_LIBRARY=${PREFIX}/lib/libpython${PY_VER}m${SHLIB_EXT}"
fi
